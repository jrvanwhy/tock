...
/// Accessor definitions and implementations for Write operations on array
/// registers.
pub fn array_write(tock_registers: &Path, name: &Ident, value: &Type, len: &LitInt) -> Parts {
    ...
    let doc = [
        ...
        format!("Returns `Err` if `index` is >= {}.", len),
    ];
    let doc_unchecked = [
    ...
    Parts {
        definition: quote_spanned! {name.span()=>
            #(#[doc = #doc])*
            fn #name_write(&self, index: usize, value: #value) -> #tock_registers::reexport::Result<(), #tock_registers::OutOfBounds> {
        ...
        mmio_impl: quote_spanned! {name.span()=>
            fn #name_write(&self, index: usize) -> #tock_registers::reexport::Option<#value> {
                // TODO: Safety comment
                unsafe {
